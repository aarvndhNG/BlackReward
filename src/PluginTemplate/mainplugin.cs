...
namespace Black-Rewards 
{
    ...
    public class AVote : TerrariaPlugin
    {
        ...
        {
            Commands.ChatCommands.Add(new Command("black.vote", Vote, "vote", "reward"));
            config = Config.Read();
        ...
        {
            args.Player.SendMessage("[BlackRewards] - API key reloaded!", Color.LightGreen);
            config = Config.Read();
            apiKey = config.apiKey;
        }
        ...
        public void Vote(CommandArgs args)
        {
            ...
            if(args.Parameters.Count == 1)
            {
                ...
                {
                    if (args.Player.HasPermission("black.admin"))
                    {
